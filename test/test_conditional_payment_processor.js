...
contract('ConditionalPaymentProcessor', function (accounts) {
    ...
    it('should split and merge positions on payout slots', async () => {
        ...
        assert.equal(await conditionalPaymentProcessor.balanceOf.call(
            keccak256(
                etherToken.address + keccak256(conditionId + padLeft(toHex(0), 64).slice(2)).slice(2)),
                accounts[buyer]),
            collateralTokenCount)
        assert.equal(await conditionalPaymentProcessor.balanceOf.call(
            keccak256(
                etherToken.address + keccak256(conditionId + padLeft(toHex(1), 64).slice(2)).slice(2)),
                accounts[buyer]),
            collateralTokenCount)
    ...
    it('should redeem payouts in more complex scenarios', async () => {
        ...
        for (var i=0; i<buyers.length; i++) {
            assert.equal(await conditionalPaymentProcessor.balanceOf.call(
                keccak256(
                    etherToken.address + keccak256(_conditionId + padLeft(toHex(i), 64).slice(2)).slice(2)),
                    accounts[buyers[i]]),
